...
Prefab, material, and mesh references are stored in a `StorageInjection`
ScriptableObject. This keeps Unity asset references inside Unity assets while
still allowing systems and MonoBehaviours to resolve them through
`IAssetStorage`.
...
services, or asset storage. Core simulation state should remain in ECS
